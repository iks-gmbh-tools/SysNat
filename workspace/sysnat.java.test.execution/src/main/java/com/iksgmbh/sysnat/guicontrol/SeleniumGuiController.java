...
public class SeleniumGuiController implements GuiControl 
{
	...
	public void openGUI() {
		...
		if (ExecutionInfo.getInstance().getBrowserTypeToUse() != BrowserType.CHROME) {
			// maximize window to void possible visibility problems of elements in popups
			//webDriver.manage().window().maximize();  // does not work with current geckodriver
			performDoubleClickOnPosition(80,15);  // workaround
		}
	}

	@Override
	public void init(String targetLoginUrl) {
		if (webDriver == null) {
			openGUI();
		}
	...
	{
		List<WebElement> findElements = webDriver.findElements(By.className("contractLinkRow"));
...
//        return rows != null ? rows.size() : 0;
    ...
    public void selectComboboxEntry(String elementIdentifier, String value)  
	{
        Select dropdown = new Select(retrieveElement(elementIdentifier));
        try {			
        	dropdown.selectByValue(value); // first try id of value
		} catch (NoSuchElementException e) {
