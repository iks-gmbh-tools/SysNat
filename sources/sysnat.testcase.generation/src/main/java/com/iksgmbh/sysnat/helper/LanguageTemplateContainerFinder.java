...
{	
	...
	private static HashMap<String, Class<?>> findCandidates(final String testApplicationName) 
	{
		...
		String applicationSpecificDirecory = System.getProperty("sysnat.languageTemplateContainer.source.dir") + "/" + testApplicationName.toLowerCase();
		...
		applicationSpecificDirecory = SysNatFileUtil.findAbsoluteFilePath(applicationSpecificDirecory);
		final List<File> result = FileFinder.searchFilesRecursively(applicationSpecificDirecory, ".java");

		String commonDirectory = System.getProperty("sysnat.languageTemplateContainer.source.dir") + "/common";
		commonDirectory = SysNatFileUtil.findAbsoluteFilePath(commonDirectory);
