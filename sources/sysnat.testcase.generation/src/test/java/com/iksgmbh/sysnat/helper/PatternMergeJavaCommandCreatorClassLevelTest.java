...
public class PatternMergeJavaCommandCreatorClassLevelTest 
{
	private PatternMergeJavaCommandCreator cut = new PatternMergeJavaCommandCreator(null, null, "SubFolderTestApplication");
	...
	@Test
	public void createsCodeForTestDataWithMultipleSingleValues() 
	{
		// arrange
		final HashMap<Filename, List<LanguageTemplatePattern>> languageTemplateCollection = 
				LanguageTemplateCollector.doYourJob(createJavaFieldData("testAppForTestData"));
		final HashMap<Filename, List<LanguageInstructionPattern>> languageInstructionCollection = 
				LanguageInstructionCollector.doYourJob("TestAppForTestData");
		
		// act
		HashMap<Filename, List<JavaCommand>> result = PatternMergeJavaCommandCreator.doYourJob(languageTemplateCollection, 
                                                                                                 languageInstructionCollection,
                																			     "TestAppForTestData");
		
		// arrange
		Filename firstKey = result.keySet().iterator().next();
		assertEquals("Java Command", "languageTemplateContainerTestImpl.setTestData(\"FirstName=Bob, Street=First Avenue, StreetNo=111\");", 
				                     result.get(firstKey).get(0).toString());
		assertEquals("Java Command", "languageTemplateContainerTestImpl.setTestData(\"Age=12, LastName=Wilson\");", 
                                     result.get(firstKey).get(1).toString());
	}
