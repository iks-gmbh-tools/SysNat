...
public class LanguageTemplatesBasics_TestApp2 extends LanguageTemplateBasics
{
	...
	private NewPageNamePageObject newPageNamePageObject;
	...
	public LanguageTemplatesBasics_TestApp2(ExecutableExample anXX) 
	{
		...
		this.newPageNamePageObject = createAndRegister(NewPageNamePageObject.class, executableExample);
		this.setCurrentPage(loginPageObject);  // if not static use: setCurrentPage(findCurrentPage());

		// define page change events
		addPageChangeEvent(new PageChangeEventBuilder().from(newPageNamePageObject).via(EventType.ButtonClick).on("aButton").to(mainPageObject).build());
		addPageChangeEvent(new PageChangeEventBuilder().from(mainPageObject).via(EventType.ButtonClick).on("anotherButton").to(newPageNamePageObject).waiting("aLabel").build());
