...
public class LanguageTemplatesBasics_TestApp2 extends LanguageTemplateBasics
{
	...
	public LanguageTemplatesBasics_TestApp2(ExecutableExample anXX) 
	{
		...
		addPageChangeEvent(new PageChangeEventBuilder().from(newPageNamePageObject).via(EventType.ButtonClick).on("aButton").to(mainPageObject).wait(1000).build());
		addPageChangeEvent(new PageChangeEventBuilder().from(mainPageObject).via(EventType.ButtonClick).on("anotherButton").to(newPageNamePageObject).build());
